var Common = {
     ...
     * @param {redirct url}
     */
     login : function(loginUrl ,url) {

    	 var from_url = (url != "")? url : location.href;

    	 location.href = loginUrl + "?from_url="+escape(from_url);
     },
     /**
     * 로그인 체크(메세지 포함, 세션키존재시:true)
     * @param {redirct url, sessionKey}
     */
     checkLogin : function(loginUrl, url, sessionKey) {
    ...
    chkPasswdDanger: function(msg){
    	if(msg.length > 16 && msg.length < 10){
			return false;
